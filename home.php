...
//                           $result = plusfunds("feebee",$BTC,$CURR_Selling_Amount_Fee);
              ...
              //          }
                  ...
                  if(!mysql_query("INSERT INTO buy_orderbook (id, date, ip, username, action, want, initial_amount, amount, rate, processed, trade_id, trade_with) VALUES ('','$date','$ip','$user_session','buy','$BTC','$PST_Order_Amount','$PST_Order_Amount','$PST_Order_Rate','1','".$my_coins->getTradeId()."','$BTCRYX')")) {
                     $Trade_Message = "System error.";
                  } else {
   ...
   if($PST_Order_Amount) {
      if($PST_Order_Rate) {
         ...
         if($PST_Order_Amount>="0.00001") {
            ...
            $Trade_Message = "You are trying to sell more '.$BTCRYXS.'s then you have!";
         }
      } else {
