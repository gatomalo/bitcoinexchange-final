   ...
   if($withdraw_address) {
      if($withdraw_amount) {
         ...
         if($withdraw_amount<=$Bitcrystalxd_Balance[$iid]) {
            ...
            $Bitcrystalxd_Withdraw_From = $Bitcrystalxd[$iid]->sendtoaddress($withdraw_address,(float)$true_withdraw_amount);
            if($Bitcrystalxd_Withdraw_From) {
               $result = minusfunds($user_session,$my_coins->coins_names_prefix[2+$cid],$withdraw_amount);
               $result = plusfunds($FEEBEE,$my_coins->coins_names_prefix[2+$cid],$fee);        // add fee to feebee account
         ...
         <td align="center" valign="top" style="padding: 5px;" nowrap>
            <table>
               <tr>
                  ...
                  <td align="center" style="padding: 2px; font-weight: bold; color: #666666;" nowrap><?php echo $Bitcrystalxd_Account_Address[$iid]; ?></td>
               </tr><tr>
                  <?php echo '<td align="left" style="padding: 2px; padding-left: 20px;">Deposits must have 6 confirmations to become active. There is a fee of '.$my_coins->coins[$my_coins->coins_names[2+$cid]]["fee"].' '.$my_coins->coins_names[2+$cid].'s to make a withraw.</td>'; ?>
               </tr>
            </table>
            </center>
         </td>
